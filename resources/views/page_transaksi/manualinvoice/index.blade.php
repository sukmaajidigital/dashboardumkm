            ...
            <tbody>
                @foreach ($manualinvoices as $manualinvoice)
                    <tr>
                        ...
                        <td>{{ $manualinvoice->tanggal }}</td>
                        <td>{{ $manualinvoice->source->sumber_transaksi }}</td>
                        <td>{{ $manualinvoice->customer->nama_customer }}</td>
                        <td>{{ $manualinvoice->invoicenumber }}</td>
                        <td>{{ 'Rp. ' . number_format($manualinvoice->last_total, 0, ',', '.') }}</td>
            ...
            document.addEventListener('DOMContentLoaded', function mainInit() {
                ...
                document.addEventListener('click', function(event) {
                    ...
                    if (event.target.id === 'addRow') {
                        let newRow = document.querySelector('#detail_manualinvoice tr').cloneNode(true);
                        ...
                        document.getElementById('detail_manualinvoice').appendChild(newRow);
                    }

                    // Hapus baris
                    if (event.target.closest('.remove-row')) {
                        const rows = document.querySelectorAll('#detail_manualinvoice tr');
                        if (rows.length > 1) {
