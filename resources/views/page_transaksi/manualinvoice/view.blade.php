<div class="mx-auto bg-white p-6 shadow-lg rounded-lg">
    <button type="button" class="btn btn-primary" onclick="window.location.href='{{ route('manualinvoice.print', $manualinvoice->id) }}'">
        <span class="icon-[tabler--printer] size-8">PRINT</span>
    </button>
</div>
<div class="mx-auto bg-white p-6 mt-7">
    <div class="flex justify-between items-start mb-6">
        ...
        <div class="text-right w-1/2">
            ...
            <h5 class="text-sm font-bold">{{ $manualinvoice->invoicenumber }}</h5>
            <div class="mt-6 mb-6">
                <h5 class="text-lg font-semibold">Invoice To :</h5>
                <p class="text-sm text-gray-600">
                    {{ $manualinvoice->customer->nama_customer }}<br>
                    {{ $manualinvoice->customer->telepon }}<br>
                    {{ $manualinvoice->customer->alamat }}
                </p>
            </div>
        </div>
    </div>
    <div class="overflow-x-auto mb-6">
        <table class="w-full text-left text-sm">
            ...
            <tbody>
                @foreach ($manualinvoice->details as $index => $detail)
                    <tr class="border-b">
                        <td class="p-2">{{ $index + 1 }}</td>
                        <td class="p-2">{{ $detail->nama_produk }}</td>
        ...
        <div class="text-right mt-4">
            <div class="mb-2 flex justify-end">
                <div class="w-1/2 flex justify-between">
                    <span>SUBTOTAL:</span>
                    <span>Rp. {{ number_format($manualinvoice->total_harga, 0, ',', '.') }}</span>
                </div>
            </div>
            <div class="mb-2 flex justify-end">
                <div class="w-1/2 flex justify-between">
                    <span>DISKON:</span>
                    <span>Rp. {{ number_format($manualinvoice->diskon, 0, ',', '.') }}</span>
                </div>
            </div>
            <div class="font-bold flex justify-end">
                <div class="w-1/2 flex justify-between">
                    <span>TOTAL:</span>
                    <span>Rp. {{ number_format($manualinvoice->last_total, 0, ',', '.') }}</span>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="mt-10 text-right">
        <p class="text-sm mb-1">Kudus, {{ $manualinvoice->tanggal }}</p>
